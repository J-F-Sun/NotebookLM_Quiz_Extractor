...
当前稳定实现是 [chrome-extension](REPO_ROOT/chrome-extension)，用于处理 NotebookLM 的 iframe / blob 制品页面。它可以导出整个测验，而不只是当前题目。
...
- [chrome-extension](REPO_ROOT/chrome-extension)：主实现，推荐实际使用
- [tampermonkey](REPO_ROOT/tampermonkey)：实验 / 测试路径，当前不建议作为正式发布版本
...
The current stable implementation is the [chrome-extension](REPO_ROOT/chrome-extension). It is designed for NotebookLM's iframe / blob-based rendering model and can export the full quiz, not only the current question.
...
- [chrome-extension](REPO_ROOT/chrome-extension): main implementation, recommended for actual use
- [tampermonkey](REPO_ROOT/tampermonkey): experimental / test path, not recommended as the release target
